...
export const filterByColor = (data) => {
	...
	data.forEach((element) => {
		!allUsedColors.find((foundColor) => foundColor === element.color) &&
			allUsedColors.push(element.color)
	})
	...
	for (let i = 0; i < highlighting.areas.length; i++) {
		if (i === 0) {
			...
			if (!highlighting.areas[i].end) {
				throw new Error(
					'First Highlighting Area needs to have a sepcified end'
				)
			}
			...
			if (!highlighting.areas[i].start) {
				throw new Error(
					'Last Highlighting Area needs to have a sepcified start'
				)
			}
		} else {
			if (!highlighting.areas[i].start || !highlighting.areas[i].end) {
				throw new Error(
					'Highlighting areas in the middle need to have a specified start and end'
				)
			}
		}
	}
	...
	for (let i = 0; i < highlighting.areas.length; i++) {
		...
		if (i === 0) {
			...
			highlighting.areas[i].end < maximum &&
				allAreas.push({
					start: highlighting.areas[i].end,
					end: maximum,
					color: standardColor,
				})
		}
